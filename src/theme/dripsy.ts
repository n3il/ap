...
const buildColors = (legacy: LegacyTheme['colors']) => {
  const colors: Record<string, string> = {
    ...
    input: legacy.input,
    accent:
      typeof legacy.accent === 'string' ? legacy.accent : legacy.accent?.DEFAULT ?? legacy.accentPalette.DEFAULT,
    accentForeground:
  ...
  if (legacy.accentPalette) {
    addPaletteGroup(colors, legacy.accentPalette, 'accent', { skipDefault: true });
  }

  (['success', 'warning', 'error', 'info', 'long', 'short'] as const).forEach((key) => {
    const palette = legacy[key];
    if (palette) {
      addPaletteGroup(colors, palette as Record<string, string>, key, { skipDefault: true });
...
const buildTextVariants = (legacy: LegacyTheme) => {
  ...
  const variants: Record<string, any> = {
    body: resolveVariant('body') ?? { fontSize: 16, lineHeight: 24, fontWeight: '400', color: 'textPrimary' },
    sm: resolveVariant('body-sm') ?? { fontSize: 14, lineHeight: 20, fontWeight: '400', color: 'textPrimary' },
    xs: resolveVariant('caption') ?? { fontSize: 12, lineHeight: 16, fontWeight: '400', color: 'textSecondary' },
    lg: resolveVariant('body-lg') ?? { fontSize: 18, lineHeight: 28, fontWeight: '400', color: 'textPrimary' },
    xl: resolveVariant('h4') ?? { fontSize: 20, lineHeight: 28, fontWeight: '600', color: 'textPrimary' },
    '2xl': resolveVariant('h3') ?? { fontSize: 24, lineHeight: 32, fontWeight: '600', color: 'textPrimary' },
    h1: resolveVariant('h1') ?? { fontSize: 40, lineHeight: 48, fontWeight: '700', color: 'textPrimary' },
    h2: resolveVariant('h2') ?? { fontSize: 32, lineHeight: 40, fontWeight: '600', color: 'textPrimary' },
    h3: resolveVariant('h3') ?? { fontSize: 24, lineHeight: 32, fontWeight: '600', color: 'textPrimary' },
    h4: resolveVariant('h4') ?? { fontSize: 20, lineHeight: 28, fontWeight: '600', color: 'textPrimary' },
    display: resolveVariant('display') ?? { fontSize: 56, lineHeight: 64, fontWeight: '700', color: 'textPrimary' },
  };
