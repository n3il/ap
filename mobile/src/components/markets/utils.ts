import { TRADE_HISTORY_FALLBACK, RECENT_TRADES_FALLBACK } from './constants';
...
export const buildTradeHistoryEntries = (trades, fallbackSymbol = 'MOVE') => {
  ...
  return trades.map((trade, index) => {
    ...
    const symbol = (trade.asset ?? (trade.symbol ?? fallbackSymbol))?.toString().toUpperCase();
...
export const buildRecentTrades = (trades, fallbackSymbol = 'XPL', fallbackPrice = 0) => {
  if (!Array.isArray(trades) || trades.length === 0) {
  ...
  return trades.slice(0, 12).map((trade, index) => {
    ...
    return {
      ...
      size: sizeValue,
      time:
        formatTimeOfDay(trade.executed_at ?? trade.entry_timestamp ?? trade.created_at) ||
        '—',
