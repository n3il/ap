...
import { View, Text, TouchableOpacity, TextInput } from '@/components/ui';
...
export default function MarketOrderTicket({
  ...
  const pillColor = side === 'buy' ? colors.success.DEFAULT : colors.error.DEFAULT;
  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', backgroundColor: withOpacity(colors.background, 0.6), borderRadius: 14 }}>
          ...
          return (
            <TouchableOpacity
              ...
              style={{
                ...
                paddingVertical: 10,
                borderRadius: 14,
                backgroundColor: isActive ? withOpacity(colors.primary.DEFAULT, 0.25) : 'transparent',
              }}
            >
              <Text
                style={{
      ...
      <View style={{ flexDirection: 'row', borderRadius: 14, overflow: 'hidden', borderWidth: 1, borderColor: withOpacity(colors.border, 0.4) }}>
        {['buy', 'sell'].map((option) => {
          const isActive = option === side;
          const optionColor = option === 'buy' ? colors.success.DEFAULT : colors.error.DEFAULT;
          return (
            <TouchableOpacity
              key={option}
              ...
              style={{
                flex: 1,
                paddingVertical: 10,
                backgroundColor: isActive ? withOpacity(optionColor, 0.2) : 'transparent',
              }}
            >
              <Text
                style={{
      ...
      <View>
        <Text style={{ color: colors.text.secondary, fontSize: 12 }}>Price</Text>
        <View
          style={{
            marginTop: 6,
            borderRadius: 14,
        ...
        >
          ...
          >
            <Text style={{ color: colors.text.secondary, fontWeight: '600' }}>{asset?.quote ?? 'USDC'}</Text>
          </View>
        </View>
      </View>

      <View>
        <Text style={{ color: colors.text.secondary, fontSize: 12 }}>Amount</Text>
        <TextInput
          style={{
            marginTop: 6,
            borderRadius: 14,
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }}>
          {(side === 'buy' ? BUY_PRESETS : SELL_PRESETS).map((preset) => (
            <TouchableOpacity
              key={preset}
              onPress={() => setAmount(side === 'buy' ? preset : preset.replace('%', ''))}
              style={{
                flex: 1,
                marginHorizontal: 4,
                borderRadius: 10,
            ...
            >
              <Text style={{ textAlign: 'center', color: pillColor, fontWeight: '600' }}>{preset}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <Text style={{ color: colors.text.primary, fontWeight: '600' }}>
          {availableBalance?.toFixed ? availableBalance.toFixed(4) : availableBalance}
        </Text>
      </View>

      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        <Text style={{ color: colors.text.secondary }}>Order Value</Text>
        <Text style={{ color: colors.text.primary, fontWeight: '700' }}>{summary}</Text>
      </View>
      ...
      >
        <Text style={{ textAlign: 'center', color: colors.surface, fontWeight: '700' }}>
